...
class DNNGoogleSpeechBatchNorm3Layer(nn.Module):
    def __init__(self, partition_num=1, sample_size=4096, model_size=4096, label_num=35):
        ...
        self.temp_hidden_layer_index1 = [range(model_size)]
        self.temp_hidden_layer_index2 = [range(model_size)]
    ...
    def partition_to_list(self, coefs):
        ...
        for i in range(self.partition_num):
            end_cursor = cursor + int(coefs[i]*self.model_size) if i<self.partition_num - 1 else len(self.temp_hidden_layer_index1) #added
        ...
        cursor = 0
        for i in range(self.partition_num):
            end_cursor = cursor + int(coefs[i]*self.model_size) if i<self.partition_num - 1 else len(self.temp_hidden_layer_index2) #added
...
def dispatch_model_to_workers(args, partitioned_model, raw_model=None, sync_time=0, coefs=None):
    ...
    print(f"Coefs: {coefs}")
    # Sync time must be greater than 0 for avoiding divide by zero error in deciding new partition size.
    # Updating the partitioned model based on the new partition size.
    ...
    # Dispatching the updated model to the workers.
    if args.rank == 0:
        ...
        # if Dynamic or all the coefs are not equal
    ...
    for i, batch in enumerate(train_loader):
        ...
        if i % args.repartition_iter == 0:
            ...
            if args.dynamic:
                ...
                partitioned_model.train()
            training_round_start_time = time.time()
        ...
        # Syncing right before the repartitioning
        # Begaii might come here
        if (i + 1) % args.repartition_iter == 0 or i == len(train_loader) - 1:
            training_round_end_time = time.time()
            sync_elapsed_time = training_round_end_time - training_round_start_time
