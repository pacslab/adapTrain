...
class Model(nn.Module):
    ...
    def replace_layer(self, index, new_layer):
        """
        Replace a layer in the model at a given index with a new layer, 
        and adjust adjacent Linear and BatchNorm layers as needed.

        Args:
            index (int): The index of the layer to replace in the model's Sequential container.
            new_layer (nn.Module): The new layer to replace the old layer. Must be a Linear or BatchNorm layer.
        """
        if not (0 <= index < len(self.model)):
            raise IndexError(f"Index {index} is out of bounds for the model layers.")

        # Replace the layer
        self.model[index] = new_layer

        # Adjust the subsequent layer, if applicable
        if index + 1 < len(self.model):
            next_layer = self.model[index + 1]
            
            # If the next layer is Linear, adjust its in_features
            if isinstance(next_layer, nn.Linear) and hasattr(new_layer, "out_features"):
                next_layer.in_features = new_layer.out_features

            # If the next layer is BatchNorm, adjust its num_features
            elif isinstance(next_layer, nn.BatchNorm1d) and hasattr(new_layer, "out_features"):
                next_layer.num_features = new_layer.out_features

        # Adjust the preceding layer, if applicable
        if index > 0:
            prev_layer = self.model[index - 1]
            
            # If the previous layer is Linear, adjust its out_features
            if isinstance(prev_layer, nn.Linear) and hasattr(new_layer, "in_features"):
                new_layer.in_features = prev_layer.out_features

            # If the previous layer is BatchNorm, adjust its num_features
            elif isinstance(prev_layer, nn.BatchNorm1d) and hasattr(new_layer, "in_features"):
                prev_layer.num_features = new_layer.in_features
